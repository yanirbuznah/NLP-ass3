...
def pmi_from_matrix(matrix):
    ...
    rot_matrix = pmi_matrix.T #np.rot90(pmi_matrix, k=-1)
...
def cosine_with_sparse_matrix(matrix, fliped_matrix, u):
    ...
    for att in fliped_matrix[u].indices:
        for v in matrix[att].indices:
...
def main():
    words_to_test = ['car','bus','hospital', 'hotel','gun', 'bomb','horse','fox','table','bowl', 'guitar', 'piano']
    ...
    words = {word_to_idx[word]: count for word, count in words.items() if count >= 100}

    # write words to file
    write_count_to_file("counts_words.txt", words, idx_to_word)

    # words_to_test = ['that', 'by']
    
    # count context1
    counts, context_list = context_counter(list_of_sentences, word_to_idx, task="full_window")
    ...
    # count context2
    counts, context_list2 = context_counter(list_of_sentences, word_to_idx, task="2_window")
    ...
    # count context3
    counts, context_list3 = context_counter(list_of_sentences, word_to_idx, task="dep_type")
    ...
    write_count_to_file("counts_context_dep.txt", context_list3, idx_to_word)
