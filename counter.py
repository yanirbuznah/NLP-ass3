...
def read_table_from_file(file_name):
    df = pd.read_csv(file_name, sep='\t', header=None,converters={'strings':str})
    df.columns = ['ID', 'FORM', 'LEMMA', 'COARSE-POS', 'FINE-POS', '-', 'HEAD', 'DEP-TYPE', '-', '-']
...
def vectorize_counts(context_counts,words,threshold=100):
    vectors = []
...
def main():
    ...
    words = {words_to_idx[word]: count for word, count in words.items() if count >= 75}
    ...
    counts_matrix = vectorize_counts(counts,words)
